...
export class ConfigManager {
	...
	public registerSpriteStateAnimation(sprite: ISpriteConfig): void {
		...
		if (!this.validateFontSprite(sprite)) return;
	...
	private loadSpriteSheet(sprite: ISpriteConfig): void {
		...
		// if font sprite is not valid, we skip it to avoid error
		if (!this.validateFontSprite(sprite)) {
			return;
		}
	...
	public getStateName(
		state: string,
		font: Phaser.Types.Physics.Arcade.SpriteWithDynamicBody,
	): string {
		return `${state}-${font.texture.key}`;
	}
	...
	private validateFontSprite(sprite: ISpriteConfig): boolean {
...
export class InputManager {
	...
	public checkIsMouseInOnFont(): void {
		try {
			invoke("get_mouse_position").then((event: any) => {
				if (this.detectMouseOverFont(event.clientX, event.clientY)) {
			...
			console.log("Error in InputManager checkIsMouseInOnFont()", error);
		}
	}
	...
	private detectMouseOverFont(clientX: number, clientY: number): boolean {
		try {
			if (!this.input) {
				return false;
			}

			// if not pixel perfect, we can detect mouse over font using this (with loop through fonts array)
			// return Phaser.Geom.Rectangle.Contains(this.fonts[0].getBounds(), clientX, clientY);
			...
			console.log("Error in InputManager detectMouseOverFont()", error);
			return false;
		}
	}
}
